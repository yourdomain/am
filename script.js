$(document).ready(function () {
    ...
    // Delete an association with confirmation
    function deleteAssociation(id) {
        ...
        const confirmDelete = confirm(
            `Are you sure you want to delete the association "${association.name}"? This action will also delete all its members and cannot be undone.`
        );

        if (confirmDelete) {
            const updatedAssociations = associations.filter((a) => a.id !== id);
            localStorage.setItem(associationKey, JSON.stringify(updatedAssociations));
            localStorage.removeItem(`${memberPrefix}${id}`);
            searchAssociations(); // Reload the associations list after deletion
            alert(`Association "${association.name}" has been successfully deleted.`);
        }
    }
    ...
    function searchAssociations() {
        const searchTerm = $('#search-association').val().toLowerCase();
        const associations = JSON.parse(localStorage.getItem(associationKey));
        $('#associations-list').empty();
        ...
        associations
            .filter((association) =>
                ...
                association.reg.toLowerCase().includes(searchTerm) ||
    ...
    $('#search-association').on('input', searchAssociations);
...
function deleteMember(id) {
    ...
    const confirmDelete = confirm(
        `Are you sure you want to delete the member "${member.name}"? This action cannot be undone.`
    );

    if (confirmDelete) {
        const updatedMembers = members.filter((m) => m.id !== id);
        localStorage.setItem(membersKey, JSON.stringify(updatedMembers));
        searchMembers(updatedMembers); // Reload the members list after deletion
        alert(`Member "${member.name}" has been successfully deleted.`);
    }
}
    ...
    function searchMembers() {
        ...
        members
            .filter((member) =>
                ...
                member.community.toLowerCase().includes(searchTerm)
            )
            .forEach((member) => {
                const photo = member.photo || placeholderImage;
                $('#members-list').append(`
                <div class="member-card">
                    <div class="member-details">
                        <img src="${photo}" alt="${member.name}" class="member-thumbnail">
                    ...
                    <div class="member-actions">
                        <button onclick="editMember('${member.id}', '${associationId}')">Edit</button>
                        <button onclick="deleteMember('${member.id}', '${associationId}')">Delete</button>
                    </div>
                </div>
            `);
            });
    ...
    // Attach the search event listener dynamically when loading members
